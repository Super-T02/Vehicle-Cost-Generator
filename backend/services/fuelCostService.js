...
exports.getAllCostItems = (req, callback) => {
	...
	fuelCostModel.getAllCostItems(query, (err, data) => {
		if (err) {
			callback(err, null);
...
exports.getCostItem = (req, callback) => {
	fuelCostModel.getCostItem(req.id, (err, data) => {
		if (err) {
			callback(err, null);
		} else if (data.length === 0) {
			callback(null, null);
...
exports.createCostItem = (req, callback) => {
	...
	fuelCostModel.addCostItem(costItem, (err, data) => {
		if (err) {
			callback(err, null);
		} else {
			callback(null, data);
...
exports.deleteCostItem = (req, callback) => {
	fuelCostModel.deleteCostItem(req.id, (err, data) => {
		if (err) {
			callback(err, null);
		} else {
			callback(null, data);
...
exports.updateCostItem = (req, callback) => {
	...
	fuelCostModel.modifyCostItem(req.id, costItem, (err, numReplaced) => {
		if (err) {
			callback(err, null);
...
exports.checkPost = async (req, res, next) => {
	...
	await check('consumption')
		.if(body('consumption').exists())
...
const existsID = async (id) => {
	return new Promise((resolve, reject) => {
		fuelCostModel.findID(id, (err, result) => {
			if (err) {
